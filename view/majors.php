...
<main id="main" class="main">
    ...
    <section class="section">
                                    ...
                                    ?>
                                        <?php while ($rows = mysqli_fetch_assoc($getdata)) { ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <?php
                                                    $getcatblog = mysqli_query($conn, "SELECT b.* from majors_detail a left join users b on a.user_id=b.id where a.majors_id='" . $rows['id'] . "'");
                                                ...
                                                <td>
                                                    <?php
                                                    $getins = mysqli_query($conn, "SELECT b.* from instructors a left join users b on a.user_id=b.id where a.majors_id='" . $rows['id'] . "'");
        ...
        <?php } else {
            $getrole = mysqli_query($conn, "SELECT b.* from user_role a left join users b on a.user_id=b.id where a.role_id='3' ");
            ...
            if (isset($_GET['tid'])) {
                ...
                $getdata = mysqli_query($conn, "SELECT a.* from majors a where a.id = '$tid'");
                $rows = mysqli_fetch_assoc($getdata);
                $name = $rows['name'];
                $is_active = $rows['is_active'];
                $getdata_detail = mysqli_query($conn, "SELECT * from majors_detail where majors_id = '$tid'");
                ...
                <div class="tab-pane fade active show" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                    <div class="row">
                        <div class="col-9">
                            <div class="card  overflow-auto effectup">
                                <div class="card-body pb-0">
                                    <input type="hidden" id="tid" name="tid" class="majorsform" value="<?= $tid; ?>">
                                    <h5 class="card-title"><?= $label; ?> Form</h5>
                                    <div class="row g-3">
                                        <div class="col-md-12 mb-2">
                                            <div class="form-floating">
                                                <input type="text" class="form-control majorsform" name="name" id="name" value="<?= $name; ?>" placeholder="Name" required>
                                                <label for="name">Name</label>
                                            </div>
                                        </div>
                                        <div class="col-md-12">
                                            <div class="form-floating mb-5">
                                                <select class="form-select majorsform" id="is_active" aria-label="is_active">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-6" align="right">
                                            <button class="btn btn-primary mt-3">
                                                <a class="icon text-white" href="#" id="add_item"><i class="fe fe-plus"></i> Add Pic</a>
                                            </button>
                                        </div>
                                    </div>
                                                ...
                                                <?php if ($checkdetail > 0) { ?>
                                                    <?php foreach ($rowsdetail as $rowd) { ?>
                                                        <tr id="sub_row<?php echo $item_counter; ?>">
                                                            <td class="pl-0">
                                                                <select class="select2tags custom-select user_id form-control majorsdetailform<?php echo $item_counter; ?>" id="user_id<?php echo $item_counter; ?>" name="user_id[]" counter="<?php echo $item_counter; ?>">
                                                                    <option value="">Choose Pic</option>
                                                                    <?php if ($numrole > 0) { ?>
                                                                        <?php foreach ($rowrole as  $rowser) { ?>
                                                                            <option value="<?= $rowser['id']; ?>" <?php if ($rowser['id'] == $rowd['user_id']) echo 'selected'; ?>><?= $rowser['name']; ?></option>
                                                                        <?php } ?>
                                                                    <?php } ?>
                                                                </select>
                                                            </td>
